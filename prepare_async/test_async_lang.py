...
RUMAGENT_URL = "http://211.188.53.220:2025"

class AutoSearchRequest(BaseModel):
    query: str

async def run_rumagent_stream(query: str):
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        ...
        response = await client.post(f"{RUMAGENT_URL}/runs", json=request_data)
        ...
        async with client.stream('GET', f"{RUMAGENT_URL}/threads/{thread_id}/runs/{run_id}/stream", headers=headers) as stream_response:
...
async def main():
    ...
    await run_rumagent_stream("미래에셋증권 자사주 소각 진짜야?")
